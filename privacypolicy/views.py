...
class call_model(APIView):
	...
	def calculateScore(self, gradedData):
		...
		for i in range(len(gradedData)):
			...
			if sentenceNum == 0:
				...
				scores.append(int(catScore / sentenceNum * 10))
	...
	def get(self, request):
		if request.method == 'GET':
			...
			finalScores = self.calculateScore(gradedData)
			# print(gradedData[0], "\n", gradedData[1], "\n", gradedData[2], "\n", gradedData[3], "\n", gradedData[4], "\n", gradedData[5], "\n")
			print("Graded Successfully")

			return render(request, "output.html", {'pageContent' : gradedData, 'lines' : pageSize, 'finalScores' : finalScores, 'filteredData' : filteredData})
